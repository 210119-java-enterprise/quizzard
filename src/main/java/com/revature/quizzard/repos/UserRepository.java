...
public class UserRepository implements CrudRepository<User> {
    ...
    public Set<User> findUsersByRole(Role role) {
        ...
        try (Connection conn = ConnectionFactory.getInstance().getConnection()) {
            ...
            String sql = "SELECT * FROM app_users WHERE role_id = ?";
    ...
    public void confirmAccount(int confirmedUserId) {
        ...
        try (Connection conn = ConnectionFactory.getInstance().getConnection();) {
            ...
            String sql = "UPDATE app_users SET confirmed = ? WHERE user_id = ?";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, 1);
    ...
    public void save(User newObj) {
        ...
        try (Connection conn = ConnectionFactory.getInstance().getConnection()) {
            ...
            String sql = "INSERT INTO app_users VALUES (0, ?, ?, ?, ?, ?, ?, 0)";
    ...
    public void update(User updatedObj) {
        ...
        try (Connection conn = ConnectionFactory.getInstance().getConnection();) {
            ...
            String sql = "UPDATE app_users SET username = ?, password = ?, email = ?, first_name = ?, last_name = ? " +
                    "WHERE user_id = ?";
            PreparedStatement pstmt = conn.prepareStatement(sql);
